...
class SitemapsController extends FrontController
{
	...
	public function index()
	{
		foreach ($this->countries as $item) {
			...
			Sitemap::addSitemap(countryUrl($country, $country->icode . '/sitemaps.xml'));
		}
	...
	public function site($countryCode = null)
	{
		...
		Sitemap::addSitemap(countryUrl(collect($country), $country->icode . '/sitemaps/pages.xml'));
		Sitemap::addSitemap(countryUrl(collect($country), $country->icode . '/sitemaps/categories.xml'));
		Sitemap::addSitemap(countryUrl(collect($country), $country->icode . '/sitemaps/cities.xml'));
		
		$countPosts = Post::verified()->countryOf($country->code)->count();
		if ($countPosts > 0) {
			Sitemap::addSitemap(countryUrl(collect($country), $country->icode . '/sitemaps/posts.xml'));
		}
