...
/**
 * Get URL related to the given country
 *
 * @param $country
 * @param string $path
 * @return \Illuminate\Contracts\Routing\UrlGenerator|string
 */
function countryUrl($country, $path = '')
{
	if (
		!($country instanceof \Illuminate\Support\Collection)
		|| !$country->has('code')
	) {
		return url('/');
	}
	...
	if ($country->has('languages')) {
		...
		if ($countryLang->has('abbr')) {
			$defaultLangCode = '/' . $countryLang->get('abbr');
		}
	} else {
		// From XML Sitemaps
		if ($country->has('locale')) {
			$defaultLangCode = '/' . $country->get('locale');
		}
	}
	...
	if (isset($domain['url']) && !empty($domain['url'])) {
		...
		$url = rtrim($domain['url'], '/') . $defaultLangCode;
		$url = $url . ((!empty($path)) ? '/' . $path : '');
	} else {
		$url = rtrim(env('APP_URL', ''), '/') . $defaultLangCode;
		$url = $url . ((!empty($path)) ? '/' . $path : '');
		...
		if (!config('settings.domain_mapping.share_session')) {
			...
			if (!empty($domain)) {
				...
				if (isset($domain['name']) && !empty($domain['name'])) {
					\Config::set('app.name', $domain['name']);
					\Config::set('settings.app.app_name', $domain['name']);
				}
				if (isset($domain['logo']) && !empty($domain['logo'])) {
					if (\Storage::exists($domain['logo'])) {
						\Config::set('settings.app.logo', $domain['logo']);
					}
				}
			}
		}
