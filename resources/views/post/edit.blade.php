						...
						<div class="row">
							<div class="col-sm-12">
								<form class="form-horizontal" id="postForm" method="POST" action="{{ url()->current() }}" enctype="multipart/form-data">
									...
									<fieldset>
										<!-- COMPANY -->
										<div class="content-subheading hide" style="margin-top: 0;">
										...
										<div class="form-group required <?php echo (isset($errors) and $errors->has('description')) ? 'has-error' : ''; ?>">
                                            <div class="col-md-12" style="position: relative; float: right; padding-top: 10px;">
												...
												<textarea class="form-control" id="description" name="description" rows="10">{{ old('description', $post->description) }}</textarea>
										...
										<div class="form-group required <?php echo (isset($errors) and $errors->has('required_skill')) ? 'has-error' : ''; ?>">
                                            <div class="col-md-12">
												...
												<input id="required_skill" name="required_skill" placeholder="{{ 'Add skills by Comma Separater' }}" class="form-control input-md"
													   type="text" value="{{ old('required_skill', $post->required_skill) }}">
											</div>
										</div>
											...
											<div class="col-md-6">
												...
												<select name="exp_month" id="exp_month" class="form-control selecter">
													...
													@foreach (range(0,11) as $month)
														<option value="{{ $month }}"
																@if (old('exp_month', $post->exp_month)==$month)
																	selected="selected"
																@endif
														> {{ $month }} </option>
													@endforeach
										...
										<div class="form-group required <?php echo (isset($errors) and $errors->has('category_id')) ? 'has-error' : ''; ?>">
											...
											<div class="col-md-6">
												...
												<select name="category_id" id="parentId" class="form-control selecter">
													<option value="0" data-type=""
															@if (old('category_id' , $post->category_id)=='' or old('category_id')==0)
																selected="selected"
															@endif
													> {{ t('Select a category') }} </option>
													@foreach ($categories as $cat)
														<option value="{{ $cat->tid }}" data-type="{{ $cat->type }}"
																@if (old('category_id', $post->category_id)==$cat->tid)
																	selected="selected"
																@endif
														> {{ $cat->name }} </option>
													@endforeach
											...
											<div class="col-md-6">
												...
												<select name="sub_category_id" id="subcategoryId" class="form-control selecter">
													<option value="0"
															@if (old('sub_category_id', $post->sub_category_id)=='' or old('sub_category_id')==0)
																selected="selected"
															@endif
													> {{ t('Select a sub-category') }} </option>
												</select>
										...
										<div id="postTypeBloc" class="form-group required <?php echo (isset($errors) and $errors->has('job_function')) ? 'has-error' : ''; ?>">
											<div class="col-md-12">
												<label class="control-label">{{ 'Job Function' }} <sup>*</sup></label>
										...
										<div class="form-group required <?php echo (isset($errors) and $errors->has('qualification')) ? 'has-error' : ''; ?>">
                                            <div class="col-md-12">
												...
												<input id="qualification" name="qualification" placeholder="{{ 'Qualification' }}" class="form-control input-md"
													   type="text" value="{{ old('qualification', $post->qualification) }}">
											</div>
										</div>
										...
										<div id="salaryBloc" class="form-group <?php echo (isset($errors) and $errors->has('salary_min')) ? 'has-error' : ''; ?>">
											...
											<div class="col-md-6">
												...
												<div class="input-group">
													<input id="employment_type" name="employment_type" class="form-control" placeholder="{{ t('Employment Type') }}" type="text" value="{{ old('employment_type',$post->employment_type) }}">
												</div>
											</div>
										...
										*/
												...
												<div class="col-md-8">
													<div class="input-group">
														...
														<input id="contact_name" name="contact_name" placeholder="{{ t('Contact Name') }}"
														   class="form-control input-md" type="text" value="{{ old('contact_name', $post->contact_name) }}">
													</div>
												</div>
											</div>
										@endif
										...
										<div id="cityBox" class="form-group required <?php echo (isset($errors) and $errors->has('city_id')) ? 'has-error' : ''; ?>">
											...
											<div class="col-md-6">
												...
												{{ Form::select('district_id', array('0'=>'Select')+$subadmin2->toarray(),$post->district_id,array('class'=>'orm-control sselecter', 'id'=>'districtId')) }}
											</div>
										</div>
										...
										<div id="job_type" class="form-group required <?php echo (isset($errors) and $errors->has('job_type')) ? 'has-error' : ''; ?>">
											...
											<div class="col-md-6">
												...
												<div>
													@foreach ($postTypes as $postType)
														{!! Form::radio('post_type_id', $postType->tid, ($post->post_type_id == $postType->tid) ? true : false , array('required'=>true)); !!}	{{ $postType->name }}
													@endforeach
											...
											<div class="col-md-6">
												...
												{{ Form::select('contract_type', array('0'=>'Select','market needs'=>'market needs', 'determined work'=>'determined work','increase activity'=>'increase activity'),$post->contract_type,array('class'=>'orm-control selecter', 'id'=>'contract_type')) }}	
										...
										<div class="form-group required <?php echo (isset($errors) and $errors->has('job_type')) ? 'has-error' : ''; ?>">
											<div class="col-md-4">
												...
												{{ Form::select('job_gender', array('0'=>'Select','m'=>'Male', 'f'=>'Female','o'=>'other'), $post->job_gender, array('class'=>'orm-control selecter', 'id'=>'job_gender')) }}
											</div>
											<div class="col-md-4">
												<lable class="control-label">Max Age</lable>
												{!! Form::select('max_age',range(16,60), $post->max_age,array('class'=>'form-control selecter')) !!}
											</div>
											<div class="col-md-4">
												<lable class="control-label">Min Age</lable>
												{!! Form::select('min_age',range(16,60), $post->min_age,array('class'=>'form-control selecter')) !!}
											</div>
										</div>
										...
										<div class="form-group required <?php echo (isset($errors) and $errors->has('job_type')) ? 'has-error' : ''; ?>">
											...
											<div class="col-md-4">
												...
												{!! Form::select('edu_level',array(''=>'Select Education','A'=>'A','B'=>'B'), $post->edu_level,array('class'=>'form-control selecter','multiple'=>'multiple')) !!}			
											</div>
											<div class="col-md-4">
												<lable class="control-label">Degree</lable>
												{!! Form::select('degree',array(''=>'Select Degree','BE'=>'BE','BCOM'=>'bcom'), $post->degree, array('class'=>'form-control selecter','multiple'=>'multiple')) !!}
											</div>
											<div class="col-md-4">
												<lable class="control-label">School</lable>
												{!! Form::select('school',array(''=>'Select School','10'=>'10','12'=>'12'), $post->school, array('class'=>'form-control selecter','multiple'=>'multiple')) !!}
											</div>
										</div>	
										...
										<div class="form-group required <?php echo (isset($errors) and $errors->has('job_type')) ? 'has-error' : ''; ?>">
											<label class="col-md-2">Select Package <sub>*</sub></label>
											<div class="col-md-4">

												{!! Form::select('package_id',array(''=>'Select Package')+$packages->toArray(), $post->package_id, array('class'=>'form-control selecter')) !!}
											</div>
