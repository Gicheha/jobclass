                    ...
                    <tr>
                        ...
                        <td class="text-right">
                            @if ($plugin->has_installer)
                                @if ($plugin->installed)
									@if ($plugin->activated)
										<a class="btn btn-xs btn-success btn-uninstall" href="{{ url(config('larapen.admin.route_prefix', 'admin') . '/plugins/' . $plugin->name . '/uninstall') }}">
											<i class="fa fa-toggle-on"></i> {{ trans('admin::messages.Uninstall') }}
										</a>
									@else
										<a class="btn btn-xs btn-danger btn-install" data-name="{!! $plugin->display_name !!}" data-checkable="{{ (!empty($plugin->item_id)) ? true : false }}" href="{{ url(config('larapen.admin.route_prefix', 'admin') . '/plugins/' . $plugin->name . '/install') }}">
											<i class="fa fa-lock"></i> {{ trans('admin::messages.Activate') }}
										</a>
									@endif
                                @else
                                    <a class="btn btn-xs btn-default btn-install" data-name="{!! $plugin->display_name !!}" data-checkable="{{ (!empty($plugin->item_id)) ? true : false }}" href="{{ url(config('larapen.admin.route_prefix', 'admin') . '/plugins/' . $plugin->name . '/install') }}">
